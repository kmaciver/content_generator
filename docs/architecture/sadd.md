...
- **`production-local`**: nginx is the only *application* entry point; uWSGI over unix socket; `next build && next start`; no source mounts; JSON logs; resource limits (`mem_limit` on renderer/chromium); restart policies `unless-stopped`. **[AMENDED M5]** Originally "no store ports exposed". Postgres now publishes on **`127.0.0.1` only**, so a GUI client (DBeaver, psql) works against the profile a developer actually leaves running — the fidelity loop was otherwise the one where you could not look at your own data. Loopback rather than the dev profile's all-interfaces bind: it is everything a local client needs and nothing a laptop on shared wifi should offer. Redis and MinIO stay closed; the containers reach all three over the compose network regardless, so nothing about the topology under test changes.
